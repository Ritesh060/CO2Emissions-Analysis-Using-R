...
model_data <- dataset %>%
  select(`Engine.Size.L.`, Cylinders, `Fuel.Consumption.City..L.100.km.`, `CO2.Emissions.g.km.`)
...
lm_model <- lm(`CO2.Emissions.g.km.` ~ `Engine.Size.L.` + Cylinders + `Fuel.Consumption.City..L.100.km.`, data = model_data)
...
plot(model_diagnostics$`Fuel.Consumption.City..L.100.km.`, model_diagnostics$.resid, main = "Residuals vs. Fuel Consumption City")
abline(h = 0, col = "red")
